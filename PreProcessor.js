...
fs.readFile('ClinVar.8.2019.txt', function (err, fileContents) {
        ...
        createLabSpecificInterpPairsArray(interpPairsArray, labArray)
...
function createLabSpecificInterpPairsArray(interpPairsArray, labArray) {
    console.log("in Create Specific Interp Pairs Array");
    for(var i = 0; i < labArray.length; i++) {
        var labSpecificArray = new Array();
        var lab = labArray[i];

        for(var j = 0; j < interpPairsArray.length; j++) {
            for(var k = 0; k < interpPairsArray[j].labInterps.length; k++) {
                if (interpPairsArray[j].labInterps[k].source == lab) {
                    labSpecificArray.push(interpPairsArray[j].labInterps)
                }
            }
        }

        writeLSInterpPairsArray(lab, labSpecificArray);
        labSpecificArray = new Array();
    }

    console.log("out of Create Lab Specific Interp Pairs Array")
}

function writeLSInterpPairsArray(lab, labSpecificArray) {
    var labSpecificArrayString = JSON.stringify(labSpecificArray);

    fs.writeFile(lab + 'LabArray.json', labSpecificArrayString, function (err) {
        if(err) return console.log(err);
    });
